...
public class FileProvider extends ContentProvider {
    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                if (entry == null) {
                    throw new IllegalArgumentException(new StringBuilder("Failed to find configured root that contains ").append(canonicalPath).toString());
                }
                ...
                return new Builder().scheme(ParamKey.CONTENT).authority(this.mAuthority).encodedPath(Uri.encode((String) entry.getKey()) + '/' + Uri.encode(path, "/")).build();
            } catch (IOException e) {
        ...
        public File getFileForUri(Uri uri) {
            ...
            int indexOf = encodedPath.indexOf(R.styleable.AppCompatTheme_spinnerDropDownItemStyle, 1);
            ...
            Object file2 = new File(file, decode2);
            try {
                File canonicalFile = file2.getCanonicalFile();
                file2 = canonicalFile.getPath();
                if (file2.startsWith(file.getPath())) {
    ...
    public String getType(Uri uri) {
        File fileForUri = this.mStrategy.getFileForUri(uri);
        int lastIndexOf = fileForUri.getName().lastIndexOf(R.styleable.AppCompatTheme_dropdownListPreferredItemHeight);
        if (lastIndexOf >= 0) {
