...
public abstract class AutoScrollHelper implements OnTouchListener {
    ...
    private static class ClampedScroller {
        ...
        public void requestStop() {
            ...
            this.mEffectiveRampDown = AutoScrollHelper.constrain((int) (currentAnimationTimeMillis - this.mStartTime), (int) HORIZONTAL, this.mRampDownDuration);
        ...
        private float getValueAt(long j) {
            ...
            if (this.mStopTime < 0 || j < this.mStopTime) {
                return AutoScrollHelper.constrain(((float) (j - this.mStartTime)) / ((float) this.mRampUpDuration), (float) RELATIVE_UNSPECIFIED, (float) DEFAULT_RELATIVE_VELOCITY) * 0.5f;
            }
            long j2 = j - this.mStopTime;
            return (AutoScrollHelper.constrain(((float) j2) / ((float) this.mEffectiveRampDown), (float) RELATIVE_UNSPECIFIED, (float) DEFAULT_RELATIVE_VELOCITY) * this.mStopValue) + (1.0f - this.mStopValue);
        }
    ...
    private float constrainEdgeValue(float f, float f2) {
        ...
        switch (this.mEdgeType) {
            ...
            case EDGE_TYPE_OUTSIDE:
                return f < 0.0f ? f / (-f2) : RELATIVE_UNSPECIFIED;
            default:
                return RELATIVE_UNSPECIFIED;
        }
    }
    ...
    private void cancelTargetTouch() {
        ...
        MotionEvent obtain = MotionEvent.obtain(uptimeMillis, uptimeMillis, XZBDevice.DOWNLOAD_LIST_FAILED, RELATIVE_UNSPECIFIED, 0.0f, HORIZONTAL);
